...
> 当前状态：docs-only / DESIGN_PROPOSED，待陈立确认。M2c-2 已验收并合并推送 main（a9eec73）；用户要求“继续”，本轮先按 19/25 号准备 M2c-3。下文推荐方案、行为差异及测试包尚未获批准，不开始实施或业务 PG 启用。
...
请评审下文方案 A 与六项决策、逐文件白名单和三类精确隔离测试目标。确认后才从最新 main 建立 `feat/m2c-governance`，纳入已确认的 29 号方案并续编 `30-M2c-3实施与验收-20260912.md`。当前方案分支只提交推送文档，业务启用及 main 合并另按实际交付评审。
...
待确认包：**方案 A + D1–D6 + 第五节逐文件范围 + 第六节三类精确 schema/文件上限、必要的临时 PG 启停和清理**。批准后只实施 M2c-3，不启动日常业务 PG。下一轮开工前再次 fetch、核实当前代码和目标；用户若选 B，则先形成其更小的完整白名单，不能拿 A 的范围只做部分却交付为完整 M2c-3。
