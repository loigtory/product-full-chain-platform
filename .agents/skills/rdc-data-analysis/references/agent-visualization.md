# Visualization Specialist 子智能体

> 交互式数据可视化、图表设计、数据叙事与发布级可视化专家。

## 目录

- [Core Expertise](#core-expertise)
- [Visualization Methodology](#visualization-methodology)
...
**重要**: 在生成任何可视化之前，**必须**首先配置中文字体，否则会出现乱码。使用跨平台自动检测：
...
# 在所有文本元素上使用 fontproperties 参数
# ax.set_title('中文标题', fontproperties=font_prop_bold, fontsize=14)
...
- **Chinese Fonts**: 必须使用 `get_chinese_font()` 自动检测并配置中文字体，通过 `FontProperties` 显式设置
...
1. **中文乱码**: 必须在代码开头使用 `get_chinese_font()` 自动检测可用字体并配置 `plt.rcParams` 和 `FontProperties`
2. **Overplotting**: Use transparency, jitter, or aggregation
3. **Scale Issues**: Use logarithmic scales or axis limits
4. **Color Problems**: Ensure colorblind-friendly palettes
5. **Label Clutter**: Rotate labels or use interactive tooltips
