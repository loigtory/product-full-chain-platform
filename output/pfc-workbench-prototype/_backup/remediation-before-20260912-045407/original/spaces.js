    ...
    if (u.productTab === 'trace')
      body = card(
        ...
        `<p class="muted">需求 → CAP → Unit → AC → 产物版本 → 作业/测试 → 证据。下方引用可定位同一条需求。</p>` +
          P.table(
            ['Unit', '负责人', '依赖', '状态', '关联 AC', '执行 / 测试'],
            r.units.map((unit, n) => [
              e(unit.id),
              e(unit.owner || r.owner),
              e(unit.dep || '—'),
              badge(unit.status, unit.status === '已实现' ? 'green' : unit.status === '进行中' ? 'cyan' : 'gray'),
          ...
          `<div class="btn-group">${b('edit-scope', '维护 Unit 与 AC')}${b('unit-owner', '分工 / 状态', {}, 'primary')}${b('export-evidence', '导出关系与证据')}</div>`,
      );
