(() => {
  ...
  P.renderMsg = (m, r) => {
    ...
    if (m.replyTo) {
      ...
      body.push(
        `<div class="msg-quote" title="引用回复 · ${e(target?.id || '')}">${e((target?.text || '原消息').slice(0, 60))}</div>`,
      );
    }
    ...
    if (m.refs?.length)
      body.push(
        `<div class="msg-refs"><span class="ref-cap">本轮使用</span>${m.refs
          .map(
            (x) =>
              `<span class="ref-chip"><span class="ref-ico">${i(x.kind === 'material' ? 'doc' : x.kind === 'code' ? 'code' : 'link')}</span>${e(x.label)}</span>`,
          )
          .join('')}</div>`,
      );
    if (m.diff && !m.diffApplied && !m.diffRejected)
      body.push(
        `<div class="diff-card"><div class="diff-head">${i('code')} 字段级差异建议</div>${m.diff.fields
          .map(
            (f) =>
          ...
          .join('')}<div class="btn-group">${b('accept-diff', '全部采纳为新版本', {}, 'primary')}${b('reject-diff', '拒绝建议')}</div><p class="source-note">采纳后生成新版本并使下游评审过期，可到画布查看差异。</p></div>`,
      );
    const actions = [];
      ...
      `<p class="muted">提测基线 ${e(P.stamp(r))}。每条 AC 保留执行结果与缺陷关系。</p>` +
        ...
        (r.testRuns?.length
          ? `<div class="test-history"><div class="rail-title">测试批次历史</div>${r.testRuns
              ...
              .map(
                (tr) =>
                  `<div class="event"><b>${e(tr.id)} · ${badge(tr.results.some((x) => x.status === 'FAIL') ? 'FAIL' : 'PASS', tr.results.some((x) => x.status === 'FAIL') ? 'red' : 'green')}</b><small>${e(P.time(tr.at))} · 基线 ${e(tr.baseline)} · ${tr.results.map((x) => x.id + ':' + x.status).join('；')}</small></div>`,
              )
              .join('')}</div>`
          : '') +
        (r.defects.length
  ...
  P.renderWork = () => {
    ...
    const refs = P.s.ui.pendingRefs || [],
      queue = P.s.ui.pendingAttachments || [];
    const composerExtra =
      (refs.length
