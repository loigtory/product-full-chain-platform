(() => {
  ...
  for (const r of Object.values(P.s.reqs)) {
    ...
    for (const m of r.messages || []) {
      ...
      if (!m.turnId) m.turnId = 'T-' + (m.id || Math.random().toString(36).slice(2, 8));
  ...
  P.uiBag = () => {
    const req = (P.r()?.id || P.s.ui.req || 'none'),
  ...
  P.releaseAction = (r, action) => {
    ...
    if (action === 'approve') {
      ...
      rel.snapshot = {
        ...
        acceptId: r.accept?.status + '@' + (r.accept?.actor || ''),
        target: rel.target,
    ...
    if (action === 'execute') {
      ...
      P.assert(
        rel.snapshot && rel.snapshot.stamp === P.stamp(r),
