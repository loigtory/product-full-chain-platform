(() => {
  ...
  A.send = () => {
    ...
    const queue = P.s.ui.pendingAttachments || [];
    ...
    const userMsg = {
      ...
      refs,
      replyTo: P.s.ui.replyTarget || null,
      status: 'ok',
      error: '',
    };
  ...
  A['pick-ref'] = (d) => {
    P.write();
    const pending = (P.s.ui.pendingRefs = P.s.ui.pendingRefs || []);
    if (d.kind === 'material') {
  ...
  A['open-attachment'] = (d) => {
    const q = P.r();
    const msg = q.messages.find((m) => (m.attachments || []).some((x) => x.id === d.id));
    const a = (msg?.attachments || []).find((x) => x.id === d.id);
    P.assert(a, '附件不存在');
    const body =
      a.type === 'image' && a.thumb
        ? `<p class="muted">${e(a.name)} · ${e(a.size)}</p><img class="att-preview-img" src="${a.thumb}" alt="${e(a.name)}">`
        : a.content
  ...
  A['accept-diff'] = () => {
    P.write();
    const q = P.r(),
      active = q.messages.filter((m) => m.diff && m.typing).at(-1) ||
        [...q.messages].reverse().find((m) => m.diff && m.status === 'ok' && !m.diffApplied);
    P.assert(active?.diff, '没有待处理的差异建议');
  ...
  A['reject-diff'] = () => {
    P.write();
    const q = P.r(),
      active = [...q.messages].reverse().find((m) => m.diff && !m.diffApplied);
    P.assert(active?.diff, '没有待处理的差异建议');
    active.diffApplied = true;
  ...
  P.enqueueFile = async (file) => {
    P.write();
    const queue = (P.s.ui.pendingAttachments = P.s.ui.pendingAttachments || []);
    ...
    try {
      if (isImage) {
        const thumb = await readThumb(file);
