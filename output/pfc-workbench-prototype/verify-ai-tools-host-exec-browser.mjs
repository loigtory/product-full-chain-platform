...
const {
  ...
  scenario,
  saveReport,
} = await import('../../server/test-data/ai-tools-remediation-fixture.mjs');
const require = createRequire(import.meta.url),
  { chromium } = require('playwright');
const report = reportFor(
  '48 host execution UI: blocked capability, visible approvals, scoped decisions, original TEXT stop/refresh',
);
...
try {
  ...
  await page.evaluate(() => {
    ...
    P.s.env = {
      ...(P.s.env || {}),
      execCapable: false,
      execution: { reason: '命令隔离待验证，文件控制待验收' },
    };
    window.__hostCalls = [];
    window.PFCAPI.api.req = async (method, url, body) => {
      window.__hostCalls.push({ method, url, body });
      if (method === 'POST') window.__hostDecided = true;
      return {
        reqId: P.r().id,
        approvals: [
          {
            ...
            state: window.__hostDecided ? 'DENIED' : 'PENDING',
  ...
  await scenario(
    report,
    'Visible pending tool decision is requirement scoped and never triggers execution',
    async () => {
      await page
        .getByRole('button', { name: '工具审批与记录', exact: true })
        .click();
      assert.match(
        await page.locator('body').innerText(),
        /此处同意不会立即执行/,
      );
      assert.match(await page.locator('body').innerText(), /extra.txt/);
      await page.screenshot({
        path: path.join(evidenceRoot, 'host-approval-' + Date.now() + '.png'),
      });
      await page.getByRole('button', { name: '拒绝', exact: true }).click();
      await page
        .getByText('pfc_write_file · extra.txt · 已拒绝', { exact: true })
        .waitFor();
      const calls = await page.evaluate(() => window.__hostCalls);
      const decisions = calls.filter((c) => c.method === 'POST');
      assert.equal(decisions.length, 1);
      assert.equal(decisions[0].body.decision, 'deny');
      assert.equal(decisions[0].body.expectedState, 'PENDING');
      await page
        .getByRole('button', { name: '关闭', exact: true })
        .last()
        .click();
      assert.match(
        await page.locator('.exec-hint').innerText(),
        /命令隔离待验证/,
      );
    },
  );
...
saveReport(report, 'host-browser');
