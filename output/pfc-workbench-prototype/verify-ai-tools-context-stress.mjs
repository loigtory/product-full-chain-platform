...
const report = {
  ...
  status: 'PASS',
  scope: '超长上下文压测：stageContext 6 条上限/30000 字符截断/长上下文产出稳定',
  tests: [],
  errors: [],
  external: [],
};
const t = (name, ok, extra) => {
  report.tests.push({ ...(extra || {}), name, status: ok ? 'PASS' : 'FAIL' });
...
async function main() {
  ...
  let target = null;
  for (const r of list) {
    if (/混合链|跨阶段/.test(r.name || '')) { target = r; break; }
  }
  if (!target) { t('定位混合链 req', false, { note: '未找到' }); return finish(); }
  ...
  const msgs = await (await fetch(BASE + '/api/reqs/' + pid + '/messages?limit=100', { headers: auth })).json();
  const items = msgs.items || [];
  const seedByStage = {};
  for (const m of items) {
    if (m.role === 'ai' && m.status === 'ok' && m.content && m.content.trim() && !seedByStage[m.stage]) {
      seedByStage[m.stage] = m.content.trim();
    }
  }
  const stages = ['req', 'design', 'dev', 'test', 'accept', 'release'];
  const pad = '（超长上下文压测填充段：产品全链路闭环作业平台要求每个阶段产出可追踪、可复核、可承接下游；本段用于放大上下文体积以验证长文本稳定性。）';
  const stageContext = [];
  ...
  // 第 7 条（应被服务端上限截断丢弃）
  stageContext.push({ stage: 'observe', title: 'observe阶段产出', text: '【第7条超限标记:observe】' + pad.repeat(10) });
  ...
  const pre = await (await fetch(BASE + '/api/reqs/' + pid, { headers: auth })).json();
  msg.expectedRevision = (pre.items || pre.req || pre).revision ?? (pre.req?.revision);
  if (msg.expectedRevision == null) {
    const one = await (await fetch(BASE + '/api/reqs/' + pid + '/messages?limit=1', { headers: auth })).json();
    const it = (one.items || [])[0];
    msg.expectedRevision = it?.revision != null ? it.revision : (one.revision ?? 1);
  }
  ...
  t('消息发送成功（非 409）', resp.ok, { status: resp.status, code: body?.error?.code });
  if (!resp.ok) {
    report.external.push({ note: '发送失败即退出，不空等', code: body?.error?.code });
    return finish();
  }

  // 轮询到 ai 终态（600s 上限）
  let last = null, aiMsg = null;
  const deadline = Date.now() + 600000;
  while (Date.now() < deadline) {
    await sleep(10000);
    const listResp = await (await fetch(BASE + '/api/reqs/' + pid + '/messages?limit=50', { headers: auth })).json();
    const arr = listResp.items || [];
    const ai = arr.filter((m) => m.role === 'ai' && m.stage === 'test').sort((a, b) => (b.id || '').localeCompare(a.id || ''))[0];
    if (ai && (ai.status === 'ok' || ai.status === 'failed')) { aiMsg = ai; break; }
    last = arr.length;
  }
  ...
  t('ai 终态 ok + real 标记', aiMsg.status === 'ok', {
    status: aiMsg.status, real: aiMsg.metadata?.real, bytes: aiMsg.content?.length,
  });
  ...
  try {
    ...
    await db.end();
    t('DB 权威 real=true 标记', rr.rows[0]?.real_flag === 'true' && rr.rows[0]?.status === 'ok', {
      dbReal: rr.rows[0]?.real_flag, dbStatus: rr.rows[0]?.status,
    });
  } catch (e) {
  ...
  const refs = stages.filter((st) => aiText.includes('【压测标记:' + st + '】') || aiText.includes(st + '阶段产出'));
  t('ai 产出引用前序（含压测标记）', marked || refs.length >= 3, { refs: refs.length });
  ...
  try {
    ...
    if (input && Array.isArray(input.stageContext)) {
      t('服务端 stageContext 上限 6 条截断', input.stageContext.length === 6, { actual: input.stageContext.length });
      t('单条 text ≤30000 字符截断', input.stageContext.every((c) => c.text.length <= 30000), {
        lens: input.stageContext.map((c) => c.text.length),
      });
      t('第 7 条（observe）被丢弃', !input.stageContext.some((c) => c.stage === 'observe'), {});
    } else {
      t('DB 可查 job.input.stageContext', false, { note: input ? 'no stageContext' : 'no job' });
    }
  } catch (e) {
  ...
  const polluted = dirty.split(/\r?\n/).filter(Boolean).some((l) => /^\s*[MADRCU?]{1,2}\s+(server|apps|packages)\//.test(l));
  t('平台仓库零污染', !polluted, {});
...
function finish() {
  ...
  const out = resolve(REPORT_DIR, 'context-stress-' + Math.floor(Date.now() / 1000) + '.json');
  writeFileSync(out, JSON.stringify(report, null, 2), 'utf8');
  console.log(JSON.stringify({ status: report.status, tests: report.tests.length, file: out, errors: report.errors }));
...
main().catch((e) => {
  ...
  report.status = 'FAIL';
  writeFileSync(resolve(REPORT_DIR, 'context-stress-' + Math.floor(Date.now() / 1000) + '.json'), JSON.stringify(report, null, 2), 'utf8');
