  ...
  wssBridge.on('connection', (ws, req) => {
    ...
    ws.on('message', (raw) => {
      ...
      if (m.type === 'job.line') {
        ...
        list.push({ cls: m.cls || 'info', text: m.text || '', at: new Date().toISOString() });
        S.lines.set(m.runId, list);
        broadcast('job.line', { runId: m.runId, cls: m.cls || 'info', text: m.text || '' });
        return;
      }
      ...
      if (m.type === 'job.done') {
        ...
        broadcast('run.status', { runId: m.runId, status: run && run.status, pct: 100 });
        return;
      }
      if (m.type === 'job.error') {
        const run = S.runs.get(m.runId);
...
module.exports = { init, dispatchJob, broadcast, bridgeList };
