(() => {
  ...
  const attIcon = (a) => {
    ...
    if (a.type === 'pdf' || a.type === 'word' || a.type === 'sheet') return 'doc';
    ...
    if (m.refs?.length)
      body.push(
        `<div class="msg-refs"><span class="ref-cap">本轮使用</span>${m.refs
          .map(
            (x, n) =>
              `<button type="button" class="ref-chip ref-link" data-action="ref-source" data-idx="${n}" title="查看引用来源">${i(x.kind === 'material' ? 'doc' : x.kind === 'code' ? 'code' : 'link')}${e(x.label)}</button>`,
          )
          .join('')}</div>`,
      );
    if (m.diff && !m.diffApplied && !m.diffRejected)
      body.push(
        `<div class="diff-card"><div class="diff-head">${i('code')} 字段级差异建议 ${b('diff-full', '完整视图', { mid: m.id }, 'ghost')}</div>${m.diff.fields
          ...
          .join('')}<div class="btn-group">${b('accept-diff', '全部采纳为新版本', { mid: m.id }, 'primary')}${b('reject-diff', '拒绝建议', { mid: m.id })}</div><p class="source-note">采纳后生成新版本并使下游评审过期，可到画布查看差异。</p></div>`,
      );
    const actions = [];
    if (m.typing && m.role === 'ai')
      actions.push(b('stop-reply', '停止答复'));
    if (m.role === 'user')
      actions.push(b('reply-message', '引用回复', { id: m.id }));
    if (m.role === 'user' && ['failed', 'stopped'].includes(m.status) && m.resent !== true)
  ...
  P.artifactCard = (r, stage) => {
    ...
    return card(
      ...
      `<p class="muted">${e(r.id)} · ${e(v.id)} · 材料基线 ${r.baseline}</p>` +
        ...
        `<div class="btn-group">${b('open-artifact', i('eye') + ' 查看 / 比较', { stage })}${b('edit-artifact', i('file') + (v.confirmed ? '创建后续草稿' : '编辑草稿'), { stage, disabled: !P.canWrite() })}${!v.confirmed || v.stale ? b('confirm-artifact', i('check') + ' 确认当前版本', { stage, id: v.id, disabled: !P.canWrite() }, 'primary') : ''}${b('review-artifact', '评审意见', { stage })}${b('download-artifact', i('download') + ' 下载当前版本', { stage })}</div>` +
        (v.comments.length
    ...
    return (
      ...
      card(
        ...
        `<p class="muted">${e(run.operation)} · ${e(r.workspace)} · 控制端 ${e(run.controller)}${run.parentId ? ' · 关联旧作业 ' + e(run.parentId) : ''}</p>` +
          `<div class="kv-row"><span>仓库</span><b>${e(git.repo)}</b></div><div class="kv-row"><span>分支 / 提交</span><b>${e(git.branch)} · ${e(git.commit)} ${git.dirty ? badge('dirty', 'orange') : badge('干净', 'green')}</b></div><div class="kv-row"><span>队列 / 预算</span><b>${queueLine} · 预算 ${Number(run.budget || 8000).toLocaleString()} 积分 · 已用 ${Math.round((run.pct || 0) / 100 * (run.budget || 8000)).toLocaleString()}</b></div>` +
        ...
        (run.status === 'SUCCEEDED' && run.files?.length
          ? card(
              ...
              `<div class="ws-files">${run.files
                ...
                .join('')}</div><p class="source-note">正式接入由 Bridge 读回真实文件与 Git 差异，此处为演示数据。</p>`,
            )
          : '') +
      card(
    ...
    card(
      ...
      `<p>负责人：${e(r.owner)}。测试通过与产品验收分别形成结论。</p>` +
        ...
        (r.accept?.status === 'ACCEPTED' && !r.tests.length
          ? P.notice('检测到验收结论与测试记录不一致：验收已通过但无测试结果，需补测试门禁证据。')
          : r.accept?.status === 'ACCEPTED' && r.defects.some((d) => d.status !== 'CLOSED')
  ...
  P.releaseCard = (r) => {
    ...
    const snapshotRows = (snap) =>
      snap
        ? [
            ['冻结基线', e(snap.stamp)],
            ['产物版本', e(snap.artifacts ? Object.values(snap.artifacts).join(' / ') : '—')],
            ['作业 / 测试', e((snap.runId || '—') + ' / ' + (snap.testRunId || '—'))],
            ['验收 / 目标', e(snap.acceptId + ' · ' + (snap.target || '—'))],
          ]
        : [];
    const changedAfterApproval =
      rel && rel.status === 'APPROVED' && rel.snapshot && rel.snapshot.stamp !== P.stamp(r);
    return card(
      i('shield') + ' 发布准备与审批',
      (rel
        ? P.table(
        ...
        (rel?.status === 'APPROVED'
          ...
          : rel?.status === 'STALE'
            ? P.notice('产物或基线已变化，原审批已失效；请重新准备并申请发布审批。', 'release-form', '重新准备')
            : rel?.status === 'FAILED'
      ...
      `<p>观察窗口：${elapsed.toFixed(1)} / ${o?.hours || 24} 小时。${r.closed ? '本需求已完成复盘。' : '完成窗口并记录指标后，可提交最终结论。'}</p>` +
        P.table(
          ...
          [
            ...
            ['冻结快照', e(r.observation?.releaseSnapshot?.stamp || r.release?.snapshot?.stamp || '未生成')],
            ['回滚准备', e(r.release?.rollback || '—')],
            ['异常 / 回滚', o?.anomaly ? e(o.anomaly.reason + ' · ' + P.time(o.anomaly.at)) : '无异常'],
          ...
          ? `<div class="rail-title">后续事项</div>` +
            P.table(
              ...
              o.followups.map((x) => [
                e(x.text),
                e(x.owner),
                badge(x.status, x.status === '已完成' ? 'green' : x.status === '进行中' ? 'cyan' : 'orange'),
              ]),
            )
          : '') +
  ...
  P.renderWork = () => {
      ...
      content += card(
        ...
        P.table(
          ...
          r.units.map((x) => [
            ...
            e(x.owner || r.owner),
            e(x.dep || '—'),
            badge(x.status, x.status === '已实现' ? 'green' : x.status === '进行中' ? 'cyan' : 'gray'),
          ]),
        ) +
    ...
    const composerExtra =
      ...
      (refs.length
        ...
        : '') +
      (queue.length
        ? `<div class="attach-tray">${queue
            .map(
              (a, n) =>
                `<div class="att-card ${a.status === 'error' ? 'err' : ''} ${a.status === 'parsing' ? 'busy' : ''}"><span class="att-ico">${i(attIcon(a))}</span><span class="att-name">${e(a.name)}</span><small>${e(a.size)}</small>${attBadge(a)}<div class="att-actions">${a.status === 'error' ? b('retry-pending', '重试', { idx: n }) : b('open-attachment', '预览', { id: a.id })}${b('remove-pending', '移除', { idx: n })}</div></div>`,
            )
            .join('')}<span class="ref-cap">待发送附件</span></div>`
        : '');
    return `<div class="work-layout" data-context="${r.id}:${stage}">${rail}<main class="chat-main"><div class="req-context-bar"><strong class="name">${e(r.id + ' · ' + r.name)}</strong>${badge('阶段 · ' + P.stageName(r.stage))}${!isCurrent ? badge('查看 · ' + P.stageName(stage), 'gray') : ''}${badge('Owner ' + r.owner, 'gray')}${b('navigate', '返回工作台', { route: 'home' })}</div><div class="stream" id="stream">${r.impact ? P.notice('材料已有新版本，需评估对当前范围和产物的影响。', 'material-impact', '评估影响') : ''}${intro}${content}${msgs}</div>${footer}<div class="composer">${composerExtra}<div class="composer-row"><button class="attch-btn" data-action="attach-menu" aria-label="添加附件" title="添加附件 / 引用上下文">${i('clip')}</button><div class="composer-box"><textarea id="chat-input" rows="1" aria-label="继续对话" placeholder="继续对话：描述意图、追问、或让 Agent 发起作业…（支持拖拽 / 粘贴截图）">${e(P.s.ui.drafts[r.id] || '')}</textarea></div><button class="send-btn" data-action="send" aria-label="发送" ${!P.canWrite() ? 'disabled' : ''}>${i('send')}</button></div></div></main>${P.renderPanel(r)}</div>`;
  };
  P.renderPanel = (r) => {
    const u = P.s.ui,
      run = P.run(r),
