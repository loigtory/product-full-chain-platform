(() => {
    ...
    commit(() => {
      ...
      o.followups = (f.followups || '')
        ...
        .map((p) => ({
          text: p[0],
          owner: p[1] || q.owner,
          status: ['待处理', '进行中', '已完成'].includes(p[2]) ? p[2] : '待处理',
        }));
      P.log(
        q,
      ...
      `<p class="muted">${rel.status} · ${e(rel.target)} · 申请 ${e(P.time(rel.expiresAt || Date.now()))}</p>` +
        (snap
          ? P.table(
              ...
              [
                ['基线', e(snap.stamp)],
                ['产物版本', e(Object.values(snap.artifacts || {}).join(' / '))],
  ...
  A['deliver-package'] = (d) => {
    const q = r();
    const rel =
      (d?.rel && (q.releaseHistory.find((x) => x.id === d.rel) || q.release)) ||
      q.release ||
      null;
    const snap = rel?.snapshot || null;
    const missing = P.blockers(q);
    const frozen = snap
    ...
    const md =
      ...
      `\n\n## 2. 测试与缺陷\n\n` +
      (q.tests.length
        ? q.tests.map((t) => `- ${t.id} ${t.text} → ${t.status}（${t.runId}）`).join('\n')
        : '尚无测试结果') +
      `\n\n缺陷：` +
      (q.defects.length ? q.defects.map((d) => `${d.id} ${d.title} ${d.status}`).join('；') : '无') +
      `\n\n## 3. 验收与发布\n\n` +
      `产品验收：${q.accept ? q.accept.status + ' · ' + q.accept.actor : '未完成'}\n` +
      `发布：${rel ? rel.id + ' · ' + rel.status : '未申请'}\n` +
      ...
      (q.closed
        ...
        : q.stage === 'observe'
          ...
          : q.accept?.status === 'ACCEPTED'
            ? '已验收，' + (rel?.status || '待发布')
            : P.stageName(q.stage) + '（' + (missing.length ? missing.join('；') : '可推进') + '）') +
      `\n\n## 5. 缺失项\n\n` +
      (missing.length ? missing.map((x) => `- ${x}`).join('\n') : '本阶段条件已满足') +
      `\n\n> 原型演示交付包；正式交付含不可变证据清单与版本指纹。草稿 / 缺失内容不冒充正式交付。`;
    P.downloadBlob(md, q.id + (rel ? '-' + rel.id : '') + '-delivery-package.md', 'text/markdown');
    P.toast('已下载交付包 ' + q.id + (rel ? '-' + rel.id : '') + '-delivery-package.md');
    P.log(q, '交付包下载', (rel ? rel.id + ' · ' : '实时 · ') + (snap ? '快照' : '未冻结'));
  };
