(() => {
  ...
  const find = (m) =>
    Object.values(window.PFC.s.reqs)
      .filter((r) => !m.reqId || r.id === m.reqId)
      .flatMap((r) => r.runs)
      .find((r) => r._remote && r.id === m.runId);
  const render = () => {
    window.PFC.save();
  ...
  const recover = () =>
    Promise.resolve(
      window.PFC.domainView?.pg()
        ? window.PFC.domainView.sync()
        : window.PFC.domain?.syncRuns?.(),
    ).catch((e) => {
  ...
  const pgRefresh = () => {
    ...
    refreshPending = true;
    setTimeout(() => {
      window.PFC.domainView
        .sync()
        .catch((e) => window.PFC.toast('服务端回读失败：' + e.message, 'error'))
        .finally(() => {
  ...
  W.handle = (m) => {
    ...
    if (m.type === 'job.line') {
      ...
      run.lines.push({
        seq: m.seq,
        cls: ['info', 'cmd', 'warn', 'error', 'ok'].includes(m.cls)
          ? m.cls
          : 'info',
        text: String(m.text || ''),
      });
      run.lines.sort((a, b) => (a.seq || 0) - (b.seq || 0));
      if (P.domainView?.pg()) run.lines = run.lines.slice(-200);
      render();
    } else if (m.type === 'run.status') {
      const run = find(m);
      if (run) {
        if (m.revision && m.revision < (run.revision || 0)) return;
        for (const key of ['status', 'pct', 'step', 'exitCode', 'revision'])
          if (m[key] !== undefined) run[key] = m[key];
        run.verified = !['UNKNOWN', 'CANCELLING'].includes(run.status);
        render();
      }
      recover();
      ...
      render();
      P.toast(m.title, m.kind || 'info');
    } else if (m.type === 'bridge.status') {
      let bridge = P.s.bridges.find((b) => b.id === m.bridgeId);
      if (!bridge) {
        bridge = { id: m.bridgeId, name: m.name, _remote: true };
        P.s.bridges.push(bridge);
      }
      Object.assign(bridge, { status: m.status, name: m.name, _remote: true });
      render();
    }
  };
  W.connect = () => {
