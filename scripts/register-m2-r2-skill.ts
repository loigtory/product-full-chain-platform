...
import { loadM2R2SkillManifest } from './m2-r2-skill-manifest.ts';
...
const manifest = await loadM2R2SkillManifest(
...
try {
  ...
  if (existing) {
    if (
      ...
      !manifest.requiredCapabilities.every((value) =>
        existing.requiredCapabilities.includes(value),
      ) ||
      !manifest.enabledScopes.every((value) =>
        existing.enabledScopes.includes(value),
      )
    ) {
