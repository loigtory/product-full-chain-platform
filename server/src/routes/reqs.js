...
router.post('/:id/versions', async (req, res, next) => {
  try {
    ...
    if (result.error) return res.status(result.error === 'NOT_FOUND' ? 404 : 409).json({error:{code:result.error,msg:'草稿内容无效或服务端已有更新版本，请重新载入'}});
...
router.post('/:id/versions/:vid/confirm', async (req, res, next) => {
  try {
    ...
    if (r.error === 'STALE_VERSION') return res.status(409).json({error:{code:r.error,msg:'版本已更新，不能确认旧版本'}});
    if (r.error === 'NOT_FOUND')
