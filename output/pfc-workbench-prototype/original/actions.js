(() => {
  ...
  /* 数据层模式切换：local / mock / api 循环；重载后生效（M1 前端先行） */
  A['data-mode'] = () => {
    const cur = window.PFCStore ? window.PFCStore.label() : '本地存储';
    window.PFCStore && window.PFCStore.cycle();
    const next = window.PFCStore ? window.PFCStore.label() : '本地存储';
    P.toast('数据层：' + cur + ' → ' + next + '，重新载入…');
  ...
  A['open-work'] = (d) => {
    ...
    P.go({
      ...
      artifactStage: d.artifactStage ?? (d.version ? d.stage : null),
      ...
      form(
        P.field('name', '需求名称', '', 'text', '用一句话说明要解决的问题') +
          ...
          P.select(
            ...
            [
              ['new', '全新项目（从零开始）'],
              ['existing', '现有系统迭代（加载本地项目）'],
            ],
            'new',
          ) +
          (P.s.projects.length
            ? `<label class="form-field"><span>关联本地项目</span>${P.select(
                'project-id',
                '',
                P.s.projects.map((p) => [
                  p.id,
                  p.name + ' · ' + p.branch + ' · ' + p.tech.join('/'),
                ]),
                P.s.ui.lastProject || P.s.projects[0].id,
              )}<small>仅「现有系统迭代」生效：项目进入上下文，需求澄清、产物与开发作业都基于它。</small></label>`
            : '<p class="muted">项目库为空：先加载本地项目。</p>') +
          b('load-project', '加载本地项目', { from: 'requirement' }, 'ghost'),
      ),
      b('close-modal', '取消') +
  ...
  A['load-project'] = (d) => {
    ...
    P.modal(
      '加载本地项目',
      form(
        P.field('p-name', '项目名称（目录名）') +
          P.field('p-path', '本地路径', '', 'text', '如 D:\\Projects\\pfc-notify') +
          P.field('p-repo', 'Git 仓库', '', 'text', '可空') +
          P.field('p-branch', '分支', 'main') +
          P.field('p-tech', '技术栈', '', 'text', '逗号分隔，如 TypeScript,Node.js') +
          '<p class="source-note">原型模拟：不会读取真实目录。正式接入由 Bridge 在选定电脑扫描目录、Git 状态与依赖清单。</p>',
      ),
      b('save-project', '加载到项目库', { from: d?.from || '' }, 'primary') +
        b('close-modal', '取消'),
    );
  };
  A['save-project'] = (d) => {
    const f = P.form();
    P.assert(f['p-name'].trim() && f['p-path'].trim(), '填写项目名称与本地路径');
    let id;
    commit(() => {
      id =
        'PRJ-' +
        String(P.s.projects.length + 1).padStart(2, '0');
      P.s.projects.push({
        id,
        name: f['p-name'].trim(),
        path: f['p-path'].trim(),
        repo: f['p-repo'].trim() || null,
        branch: f['p-branch'].trim() || 'main',
        tech: (f['p-tech'] || '')
          .split(',')
          .map((s) => s.trim())
          .filter(Boolean),
        files: 0,
        loadedAt: new Date(P.now()).toISOString(),
        source: 'existing',
      });
      P.s.ui.lastProject = id;
      P.log(null, '本地项目加载', id + ' · ' + f['p-name'].trim());
    });
    if (d?.from === 'requirement') A['new-requirement']();
    else P.toast('已加载到项目库，可在需求或治理中心使用');
  };
  A['attach-project'] = () => {
    P.write();
    P.modal(
      '关联本地项目',
      `<p>现有系统迭代：选择项目后，澄清、产物与开发作业都基于该项目上下文。</p><div class="search-results">` +
        P.s.projects
          .map((p) =>
            b(
              'pick-project',
              e(p.name) + ' · ' + e(p.branch) + ' ' + P.badge(p.tech.join('/')),
              { id: p.id },
            ),
          )
          .join('') +
        `</div><div class="btn-group">${b('load-project', '加载本地项目', {}, 'primary')}</div>`,
    );
  };
  A['pick-project'] = (d) =>
    commit(() => {
      const pj = P.s.projects.find((p) => p.id === d.id);
      P.assert(pj, '项目不存在');
      const q = P.r();
      q.projectId = d.id;
      q.workspace = pj.name;
      P.log(q, '关联本地项目', pj.id + ' · ' + pj.name);
    });
  A['create-requirement'] = async () => {
    const f = P.form();
    P.assert(
    ...
    const existing = (f['project-source'] || 'new') === 'existing';
    const projectId = existing ? f['project-id'] || null : null;
    if (existing)
      P.assert(
        P.s.projects.some((p) => p.id === projectId),
        '请先加载并选择本地项目',
      );
    P.write();
    const created = P.domain?.enabled() ? await window.PFCAPI.api.req('POST','/api/reqs',{
      name:f.name.trim(),goal:f.goal.trim(),scope:f.scope.trim(),owner:f.owner.trim(),projectId,
    }) : null;
    let req;
    commit(() => {
      const id = created?.req.id || 'R-' + ++P.s.seq;
      P.s.seq = Math.max(P.s.seq, Number(id.replace(/^R-/,'')) || 0);
      req = P.makeRequirement(id, f.name.trim(), f.goal.trim());
      req.owner = f.owner.trim();
      if (projectId) {
        const pj = P.s.projects.find((p) => p.id === projectId);
        req.projectId = projectId;
        req.workspace = pj.name;
        req.materials[0].content = '现有系统迭代：基于 ' + pj.name + ' 做增量改造';
      }
      ...
      P.s.reqs[id] = req;
      /* 知识库自动检索：按名称/目标关键词命中，注入上下文供澄清与产物引用 */
      const hay = (req.name + ' ' + req.goal).toLowerCase();
      const hits = P.s.knowledge.filter(
        (k) =>
      ...
      req.knowledgeRefs = hits.map((k) => k.id);
      if (hits.length)
        req.materials.push({
          id: 'M-' + P.s.seq + '-KN',
          name: '知识库引用（自动检索 ' + hits.length + ' 条）',
          content:
            '自动匹配：' +
            hits.map((k) => k.title).join('；') +
            '。可在治理中心·知识库查看全文。',
          cls: '内部',
          type: 'knowledge',
        });
  ...
  A['confirm-artifact'] = (d) => P.domain?.enabled()
    ? P.domain.confirm(r(),d.stage,d.id)
  ...
  A['save-scope'] = () => {
    ...
    P.assert(
      f.cap.trim() &&
        ...
        new Set(acs).size === acs.length,
      'CAP、Unit 和 AC 均必填，名称或验收标准不能重复',
    );
    commit(() => {
      const q = r();
      q.capId = f.cap.trim();
      ...
      q.acs = acs.map((text) => {
        ...
        return hit
          ? { ...hit, text }
          : { id: 'AC-' + String(++P.s.seq).padStart(2, '0'), text };
      });
      for (const unit of q.units)
        unit.acIds = (unit.acIds || []).filter((id) =>
          q.acs.some((a) => a.id === id),
        );
  ...
  A['trace-links'] = () => {
    P.write();
    const q = r();
    P.traceEditor = { req: q.id, base: P.latest(q, 'req').id };
    P.modal(
      '维护 Unit 与 AC 关联',
      '<p class="muted">关联保存到 Unit 标识，调整列表顺序不会改变关系。保存后需重新确认范围。</p>' +
        q.units
          .map(
            (u) =>
              `<fieldset><legend>${e(u.id + ' · ' + u.name)}</legend>${q.acs.map((a) => `<label class="checklist"><input type="checkbox" data-unit="${e(u.id)}" value="${e(a.id)}" ${u.acIds?.includes(a.id) ? 'checked' : ''}>${e(a.id + ' · ' + a.text)}</label>`).join('')}</fieldset>`,
          )
          .join(''),
      b('save-trace-links', '保存关联', {}, 'primary') +
        b('close-modal', '取消'),
    );
  };
  A['save-trace-links'] = () => {
    ...
    const q = r();
    P.assert(
      P.traceEditor?.req === q.id &&
        P.traceEditor.base === P.latest(q, 'req').id,
      '范围已经变化，请重新打开关联编辑',
    );
    const links = [
      ...document.querySelectorAll('input[data-unit]:checked'),
    ].map((x) => ({ unit: x.dataset.unit, ac: x.value }));
    for (const u of q.units)
      u.acIds = links.filter((x) => x.unit === u.id).map((x) => x.ac);
    P.newVersion(q, 'req', P.latest(q, 'req').fields);
    P.log(
      q,
      'Unit / AC 关联更新',
      q.units
        .map((u) => u.id + ': ' + (u.acIds.join(',') || '待关联'))
        .join('；'),
    );
    P.save();
    P.close();
  ...
  A['unit-owner'] = () => {
    P.write();
    const q = r();
    const members = P.s.team.members.map((m) => m.name);
    P.modal(
      'Unit 分工与状态',
      `<p class="muted">负责人从团队成员中选择；状态变化留痕到时间线。成员登记在治理中心 · 团队角色。</p>` +
        form(
          q.units
            .map(
              (u, n) =>
                `<div class="unit-owner-row"><b>${e(u.id)} ${e(u.name)}</b><label class="form-field">负责人<select name="uo${n}">${members.map((m) => `<option value="${e(m)}" ${(u.owner || q.owner) === m ? 'selected' : ''}>${e(m)}</option>`).join('')}</select></label><label class="form-field">状态<select name="us${n}">${['待开始', '进行中', '已实现'].map((s) => `<option ${u.status === s ? 'selected' : ''}>${s}</option>`).join('')}</select></label></div>`,
            )
            .join(''),
        ),
      b('save-units', '保存分工与状态', {}, 'primary') +
        b('close-modal', '取消'),
    );
  };
  A['save-units'] = () => {
    ...
    commit(() => {
      q.units.forEach((u, n) => {
        const owner = f['uo' + n];
        const status = f['us' + n];
        if (owner && owner !== (u.owner || q.owner))
          P.log(q, 'Unit 分工调整', u.id + ' → ' + owner);
        if (status && status !== u.status)
          P.log(q, 'Unit 状态变化', u.id + ' → ' + status);
        if (owner) u.owner = owner;
        if (status) u.status = status;
      });
      P.log(q, 'Unit 分工保存', q.units.map((x) => x.id).join(','));
    });
  };
  A.advance = () => P.domain?.enabled() ? P.domain.advance(r()) : commit(() => P.advance(r()));
})();
