(() => {
  ...
  A['save-material'] = async () => {
    ...
    commit(() => {
      ...
      q.materials.push(m);
      if (m.status === '待影响评估') q.impact = m.id;
  ...
  A['material-impact'] = () => {
    ...
    P.modal(
      '评估材料变更影响',
      `<p>新材料可能改变范围、方案或验收标准。纳入后建立新材料基线，返回需求阶段重新评审；旧版本与旧执行记录保留。</p><p>受影响对象：需求文档、技术方案、开发作业、测试和发布审批。</p>`,
  ...
  A['apply-impact'] = () =>
    commit(() => {
      const q = r(),
        m = q.materials.find((x) => x.id === q.impact);
  ...
  A['save-scope'] = () => {
    ...
    commit(() => {
      const q = r();
      q.capId = f.cap.trim();
      q.units = units.map((name, n) => ({
        id: q.id + '-U' + (n + 1),
        name,
        status: '待开始',
      }));
      q.acs = acs.map((text, n) => ({
        id: 'AC-' + String(n + 1).padStart(2, '0'),
        text,
      }));
