...
router.get('/', async (req, res, next) => {
  try { res.json(await svc.listRuns(req.query.reqId)); } catch (e) { next(e); }
});
router.get('/:id', async (req, res, next) => {
  try {
    const result = await svc.getRun(req.params.id);
    if (!result) return res.status(404).json({error:{code:'NOT_FOUND',msg:'作业不存在；内存服务重启后需重新发起'}});
    res.json(result);
  } catch (e) { next(e); }
});
...
router.post('/', async (req, res, next) => {
  try {
    const r = await svc.createRun({ reqId: req.body?.reqId, plan: req.body?.plan, commandId: req.body?.commandId, parentId: req.body?.parentId });
    if (r.error === 'NOT_FOUND')
      return res.status(404).json({ error: { code: 'NOT_FOUND', msg: '需求不存在' } });
    if (r.error) return res.status(409).json({error:{code:r.error,msg:'请求标识已用于其他计划'}});
...
router.post('/:id/plan-approve', async (req, res, next) => {
  try {
    ...
    if (r.error === 'INVALID_RUN_STATE') return res.status(409).json({error:{code:r.error,msg:'当前作业不可审批'}});
    if (r.error === 'NOT_FOUND')
...
router.post('/:id/plan-reject', async (req, res, next) => {
  try {
    ...
    if (r.error === 'INVALID_RUN_STATE') return res.status(409).json({error:{code:r.error,msg:'当前作业不可拒绝'}});
    if (r.error === 'NOT_FOUND')
      return res.status(404).json({ error: { code: 'NOT_FOUND', msg: '作业不存在' } });
...
router.post('/:id/start', async (req, res, next) => {
  try {
    ...
    if (r.error === 'INVALID_RUN_STATE') return res.status(409).json({error:{code:r.error,msg:'当前作业不可启动'}});
    if (r.error === 'NOT_FOUND')
      return res.status(404).json({ error: { code: 'NOT_FOUND', msg: '作业不存在' } });
