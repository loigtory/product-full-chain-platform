(() => {
  ...
  A['plan-run'] = (d) => {
    P.write();
    P.current(r());
    if(P.domain?.enabled()) P.domainPlan={reqId:r().id,stamp:P.stamp(r()),parentId:d.parent || null,
      commandId:crypto.randomUUID(),text:'1) 读取项目结构与目标文件 2) 按方案实现并测试 3) 格式化、构建与汇总差异；本轮仅模拟协议，不执行真实工作区命令'};
    P.modal(
      '本地作业 · 计划审批（硬门）',
  ...
  A['reject-plan'] = () => {
    if(P.domain?.enabled()) return P.domain.submitPlan(true);
    const f = P.form();
    commit(() => {
      P.log(
        r(),
  ...
  A['run-quality-gates'] = (d) => {
    ...
    P.assert(run, '没有关联作业');
    P.assert(!run._remote,'领域作业的质量门需真实工具证据；当前仅支持回读，不生成模拟通过结果');
    commit(() => {
  ...
  A['start-run'] = (d) => P.domain?.enabled() ? P.domain.submitPlan() : commit(() => P.startRun(r(), d.parent || null));
  A['run-control'] = (d) => {
    const remote=P.run(r());
    if(remote?._remote) {
      P.write();
      if(d.control==='verify') return P.domain.readRun(remote.id);
      if(d.control==='cancel') return window.PFCAPI.api.req('POST','/api/runs/'+remote.id+'/cancel',{}).then(()=>P.domain.readRun(remote.id));
      P.assert(false,'当前领域作业暂不支持此控制操作；请使用服务端核验或停止');
    }
    if (d.control !== 'verify')
