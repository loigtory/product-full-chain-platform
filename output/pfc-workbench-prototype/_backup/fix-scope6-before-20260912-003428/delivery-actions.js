(() => {
  ...
  A['deliver-package'] = () => {
    ...
    const md =
      ...
      `基线：${P.stamp(q)}（演示，正式交付须绑定代码/构建/测试/验收快照）\n\n` +
      `## 1. 产物\n\n` +
      Object.values(q.artifacts)
        .map((vs) => {
          const v = vs.at(-1);
      ...
      `发布：${q.release ? q.release.id + ' · ' + q.release.status : '未申请'}\n` +
      `观察：${q.observation ? (q.observation.conclusion || '观察中') : '未开始'}\n\n` +
      `## 4. 缺失项\n\n` +
      (missing.length ? missing.map((x) => `- ${x}`).join('\n') : '本阶段条件已满足') +
      `\n\n> 原型演示交付包；正式交付含不可变证据清单与版本指纹。`;
    P.downloadBlob(md, q.id + '-delivery-package.md', 'text/markdown');
    P.toast('已下载交付包 ' + q.id + '-delivery-package.md');
    P.log(q, '交付包下载', q.id + '-delivery-package.md');
  };
