...
async function main() {
  const login = await fetch(BASE + '/api/auth/dev-login', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'owner' }),
  });
  ...
  for (const tg of targets) {
    const r = await (await fetch(BASE + '/api/reqs/' + tg.pid, { headers: auth })).json();
  ...
  const jobs = [];
  for (const tg of targets) {
    const msgs = await (await fetch(BASE + '/api/reqs/' + tg.pid + '/messages?limit=100', { headers: auth })).json();
    const items = msgs.items || [];
    const seeds = [];
    for (const st of ['req', 'design', 'dev']) {
      const cand = items.filter((m) => m.role === 'ai' && m.stage === st && m.status === 'ok' && m.content && m.content.trim());
      if (cand.length) seeds.push({ stage: st, title: st + '阶段产出', text: cand[cand.length - 1].content.trim().slice(0, 4000) });
    }
    jobs.push({
      ...tg,
      revision: await latestRev(tg.pid, auth),
      stageContext: seeds,
      content: '基于前序阶段产出，编制测试策略与用例清单（并行压测）：覆盖主流程、异常边界与验收映射。请引用前序中至少一个具体结论。',
    });
  }
  ...
  const postA = await fetch(BASE + '/api/reqs/' + jobs[0].pid + '/messages', {
    ...
    body: JSON.stringify({ commandId: 'par-a-' + Date.now(), expectedRevision: jobs[0].revision, stage: jobs[0].stage, mode: 'real', content: jobs[0].content, stageContext: jobs[0].stageContext }),
  });
  const rA = await postA.json();
  t('作业 A 发送成功', postA.ok || postA.status === 201, { status: postA.status, code: rA?.error?.code });

  await sleep(800);
  const postB = await fetch(BASE + '/api/reqs/' + jobs[1].pid + '/messages', {
    method: 'POST', headers: { 'content-type': 'application/json', ...auth },
    body: JSON.stringify({ commandId: 'par-b-' + Date.now(), expectedRevision: jobs[1].revision, stage: jobs[1].stage, mode: 'real', content: jobs[1].content, stageContext: jobs[1].stageContext }),
  });
  const rB = await postB.json();
  t('作业 B 发送成功（间隔 <1s）', postB.ok || postB.status === 201, { status: postB.status, code: rB?.error?.code });
  ...
  while (Date.now() < deadline && (!done.A || !done.B)) {
    await sleep(10000);
    for (const [k, j] of [['A', jobs[0]], ['B', jobs[1]]]) {
      if (done[k]) continue;
      const listResp = await (await fetch(BASE + '/api/reqs/' + j.pid + '/messages?limit=50', { headers: auth })).json();
      const arr = listResp.items || [];
      const ai = arr.filter((m) => m.role === 'ai' && m.stage === j.stage).sort((x, y) => (y.id || '').localeCompare(x.id || ''))[0];
      if (ai && (ai.status === 'ok' || ai.status === 'failed')) done[k] = { status: ai.status, at: Date.now(), id: ai.id, bytes: ai.content?.length };
    }
  }
  const tEnd = Date.now();
  const elapsedA = done.A ? Math.round((done.A.at - t0) / 1000) : null;
  const elapsedB = done.B ? Math.round((done.B.at - t0) / 1000) : null;
  t('作业 A 到达终态', !!done.A && done.A.status === 'ok', { elapsedSec: elapsedA, status: done.A?.status });
  t('作业 B 到达终态', !!done.B && done.B.status === 'ok', { elapsedSec: elapsedB, status: done.B?.status });
  if (done.A && done.B) {
    ...
    t('总耗时接近单作业（并行非串行）', Math.max(elapsedA, elapsedB) < elapsedA + elapsedB - 60, {
      elapsedA, elapsedB,
    });
  }
...
function finish() {
  ...
  const out = resolve(REPORT_DIR, 'parallel-stress-' + Math.floor(Date.now() / 1000) + '.json');
...
main().catch((e) => {
  ...
  report.status = 'FAIL';
  writeFileSync(resolve(REPORT_DIR, 'parallel-stress-' + Math.floor(Date.now() / 1000) + '.json'), JSON.stringify(report, null, 2), 'utf8');
