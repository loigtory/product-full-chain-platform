...
- The current user-confirmed development path is the original HTML/JavaScript prototype plus the Express server. Start with [19 - Roadmap](docs/planning/prototype-v3/19-开发路线图-20260912.md), [20 - M2b-2 task](docs/planning/prototype-v3/20-Codex交接提示词-20260912.md), and [21 - Implementation checkpoint](docs/planning/prototype-v3/21-M2b-2前端切领域API-20260912.md). Read all three: roadmap completion labels do not supersede the checkpoint's incomplete implementation, verification, or scope decisions. Unresolved differences must be aligned with the user before expanding implementation.
...
- Current prototype work is M2b-2, partially implemented and not accepted end to end. The additional API integration decisions recorded in 21 remain pending; approval to organize files under [22 - Cleanup scope](docs/planning/prototype-v3/22-主线收口与历史代码整理建议-20260912.md) does not approve those decisions. M2c and the old POD ledger's next route are distinct records; do not silently update either scope or status.
...
- All existing authorization, lifecycle, secrets, local binding, test-data, external-effects, release, and process-cleanup requirements still apply. File organization must not change business source, dependency manifests, migrations, or the old delivery ledger. This checkout has no commit history at this checkpoint; use verified file manifests and local before-images for recovery, and recheck Git before any later cleanup.
