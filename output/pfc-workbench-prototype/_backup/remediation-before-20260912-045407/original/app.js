(() => {
  ...
  A['open-attachment'] = (d) => {
    const q = P.r(),
      bag = P.uiBag(),
      queue = bag.atts,
      a = queue.find((x) => x.id === d.id) ||
        q.attachments.find((x) => x.id === d.id) ||
        q.messages.flatMap((m) => m.attachments || []).find((x) => x.id === d.id);
    P.assert(a, '附件不存在');
    const meta = a.meta || {};
  ...
  A['download-attachment-text'] = (d) => {
    const q = P.r(),
      bag = P.uiBag(),
      a = bag.atts.find((x) => x.id === d.id) || q.attachments.find((x) => x.id === d.id);
    P.assert(a?.content, '该附件没有可下载的文本内容');
    P.downloadBlob(a.content, a.name.replace(/\.[^.]+$/, '') + '.txt', 'text/plain');
    P.toast('已下载解析文本 ' + a.name);
  };
  A['diff-full'] = (d) => {
    const q = P.r(),
    ...
    P.modal(
      ...
      `<p class="muted">来源消息 ${e(m.id)} · ${e(P.time(m.time || m.at || Date.now()))} · 采纳后生成新版本并使下游评审过期</p>` +
        m.diff.fields
          .map(
            (f) =>
              `<section class="diff-block"><div class="diff-name">${e(f.name)}</div><div class="diff-cols"><div><h4>之前</h4><div class="doc-body dim">${e(f.before)}</div></div><div><h4>之后</h4><div class="doc-body">${e(f.after)}</div></div></div></section>`,
          )
          .join(''),
      b('accept-diff', '全部采纳为新版本', { mid: d.mid }, 'primary') +
    ...
    try {
      if (isImage) {
        ...
        if (a) {
          ...
          a.type = 'image';
          a.thumb = thumb;
          a.meta = { origin: 'image', rawInMemory: true };
        }
        P.raws[id] = raw;
      } else if (isText) {
        ...
        if (a) {
          ...
          a.status = 'ready';
          a.type = 'text';
          a.content = content.slice(0, FULL);
          a.meta = {
            origin: 'text',
            charCount: content.length,
            truncated: content.length > FULL,
            fullInMemory: true,
          };
        }
      } else {
        ...
        if (a) {
          a.status = 'ready';
          a.type = /\.(pdf)$/i.test(file.name)
            ? 'pdf'
            : /\.(docx)$/i.test(file.name)
              ? 'word'
              : 'sheet';
          /* 原件与解析分开：解析结果为演示摘要，原件二进制保留在当前会话内存 */
          a.content =
            a.type === 'pdf'
              ? '（演示解析）已接收 PDF，正文提取与结构化属于正式接入。共 12 页，覆盖需求说明、交互流程与验收清单。'
              : a.type === 'word'
                ? '（演示解析）已接收 Word 文档，正文提取与结构化属于正式接入。共 8 段 / 1,240 字，含表格 2 张。'
                : '（演示解析）已接收 Excel 工作簿，正文提取与结构化属于正式接入。工作表 3 张：需求 / 验收 / 排期，共 42 行数据。';
          a.meta = {
            origin: 'binary',
            pages: a.type === 'pdf' ? 12 : null,
            sheets: a.type === 'sheet' ? 3 : null,
            paragraphs: a.type === 'word' ? 8 : null,
            fullInMemory: false,
          };
        }
      }
    ...
    for (const q of Object.values(P.s.reqs)) {
      ...
      for (const [stage, versions] of Object.entries(q.artifacts)) {
        ...
        for (const v of vs.slice(-3).reverse())
          hit(
            e(q.id + ' · ' + P.stageName(stage) + ' · ' + v.title + ' · ' + v.id),
      ...
      for (const msg of q.messages)
        if (term && (msg.text || '').includes(term))
          hit(
            e(q.id + ' · ' + P.stageName(msg.stage) + ' · 对话 · ' + (msg.text || '').slice(0, 26) + '…'),
