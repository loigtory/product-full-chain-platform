...
const report = {
  ...
  status: 'PASS',
  scope: '跨阶段上下文传递·全链路：req→design→dev→test→release→observe 逐级携带前序产出',
  tests: [],
  errors: [],
  external: [],
};
...
const STAGES = [
  { stage: 'req', content: '请按需求阶段工作法，把「依赖与安全扫描」能力固化为可作业的需求（功能点、优先级、验收标准、范围与待确认项）。' },
  { stage: 'design', content: '请按设计阶段工作法，基于前序需求结论输出「依赖与安全扫描」的方案设计：整体架构、模块划分、关键接口与设计决策。' },
  { stage: 'dev', content: '请按开发阶段工作法，基于前序设计结论输出「依赖与安全扫描」的可执行实现计划：任务拆分、实施顺序、每项验证方式与涉及文件。' },
  { stage: 'test', content: '请按测试阶段工作法，基于前序实现计划输出「依赖与安全扫描」的测试矩阵：核心用例、场景、预期结果与自动化覆盖标记。' },
  { stage: 'release', content: '请按发布阶段工作法，基于前序测试结论输出「依赖与安全扫描」的发布检查清单与回滚方案（含灰度/放量建议）。' },
  { stage: 'observe', content: '请按观察复盘阶段工作法，基于前序发布结论输出「依赖与安全扫描」上线后的观察方案：核心指标、监控点、告警与复盘触发条件。' },
];
...
async function main() {
  ...
  for (const s of STAGES) {
    ...
    if (confirmed.length) {
      body.stageContext = confirmed.map((c, i) => ({
        stage: c.stage,
        title: c.stage + '阶段产出',
        text: c.text,
      }));
    ...
    if (![200, 201, 202].includes(m.status)) {
      let b = '';
      try { b = JSON.stringify(await m.json()); } catch {}
      t(tag + ' 发送 TEXT 作业（' + (confirmed.length ? '带 stageContext' : '首阶段') + '）', false, { status: m.status, body: b });
      report.external.push({ step: tag + '-send', status: m.status, body: b });
      continue;
    }
    t(tag + ' 发送 TEXT 作业（' + (confirmed.length ? '带 stageContext' : '首阶段') + '）', true, { status: m.status });
    ...
    if (!ai || ai.failed || !ai.content || ai.content.trim().length < 30) continue;
    const text = ai.content.trim();
    t(tag + ' 产出非空', text.length > 30, { len: text.length });
    const realFlag = await realFlagOf(ai.id);
    t(tag + ' metadata.real=true（DB 直查）', realFlag === 'true', { realFlag });

    if (confirmed.length) {
      const noUpstreamClaim = !/未提供上一步|未包含上一步|当前(对话|会话)未(提供|包含|包括)上一步|未提供.{0,10}上一步|没有.{0,10}上一步/.test(text);
      t(tag + ' 不再声明"未提供上一步内容"', noUpstreamClaim, { hasClaim: !noUpstreamClaim });
      const cites = /前序|上一步|上文|前述|前文|上述需求|所附|沿用|承接|延续|继承|发布结论|候选范围|发布门禁|(依据|基于|根据|结合|参考|沿用).{0,24}(需求|设计|实现|计划|材料|结论|草案|矩阵|清单|范围|门禁)/.test(text);
  ...
  const out = resolve(REPORT_DIR, 'stage-context-chain-' + Math.floor(Date.now() / 1000) + '.json');
...
main().catch((e) => {
  ...
  report.status = 'FAIL';
  writeFileSync(
    resolve(REPORT_DIR, 'stage-context-chain-' + Math.floor(Date.now() / 1000) + '.json'),
