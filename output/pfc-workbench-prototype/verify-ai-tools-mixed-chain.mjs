...
const EXEC_WS = 'D:/项目管理/product-full-chain-platform/.local/ai-tools-live-smoke-20260915/proj';
const REPORT_DIR = resolve(projectRoot, 'docs/quality-gate/reports/ai-tools-integration-20260914');
const report = {
  at: new Date().toISOString(),
  status: 'PASS',
  scope: 'EXEC+TEXT 混合链：req(TEXT)→design(TEXT+req)→dev(EXEC 真实执行)→test(TEXT+dev 执行结果)',
  tests: [],
  errors: [],
  external: [],
};
...
async function waitAiOk(pid, stage, auth) {
  ...
  while (Date.now() < deadline) {
    const list = await (
      await fetch(BASE + '/api/reqs/' + pid + '/messages?limit=60', { headers: auth })
    ).json();
...
async function main() {
  ...
  t('REQ ai 终态 ok', !!(ai && ai.status === 'ok' && !ai.failed), { meta: ai?.metadata || null });
  if (!ai || ai.failed || !ai.content || ai.content.trim().length < 30) return finish();
  ...
  m = await fetch(BASE + '/api/reqs/' + pid + '/messages', {
    ...
    body: JSON.stringify({
      ...
      stage: 'design',
      mode: 'real',
      stageContext: confirmed.map((c) => ({ stage: c.stage, title: c.title, text: c.text })),
    }),
  });
  ...
  t('DESIGN ai 终态 ok', !!(ai && ai.status === 'ok' && !ai.failed), { meta: ai?.metadata || null });
  if (!ai || ai.failed || !ai.content || ai.content.trim().length < 30) return finish();
  ...
  const desNoClaim = !/未提供上一步|未包含上一步|当前(对话|会话)未(提供|包含|包括)上一步/.test(desText);
  t('DESIGN 不再声明"未提供上一步内容"', desNoClaim, {});
  confirmed.push({ stage: 'design', title: '方案设计产出', text: desText });
  report.external.push({ step: 'design', len: desText.length, snippet: desText.slice(0, 200) });
  ...
  t('DEV-EXEC ai 终态 ok（执行完成回写）', !!(ai && ai.status === 'ok' && !ai.failed), {
    meta: ai?.metadata || null,
  });
  let devText = null;
  if (ai && ai.status === 'ok' && !ai.failed && ai.content && ai.content.trim().length > 0) {
    devText = ai.content.trim();
    t('DEV-EXEC 回写内容非空', devText.length > 30, { len: devText.length });
    const devReal = await realFlagOf(ai.id);
    t('DEV-EXEC metadata.real=true（DB 直查）', devReal === 'true', { realFlag: devReal });
  } else {
  ...
  try {
    ...
    testOut = execSync('node --test', { cwd: EXEC_WS, stdio: 'pipe', encoding: 'utf8', timeout: 60000 });
    testPass = /pass/.test(testOut.toLowerCase()) && !/fail/.test(testOut.toLowerCase().split(' ').slice(0, 40).join(' '));
    // node:test 输出格式：tests 1, pass 1, fail 0
    testPass = /tests \d+.*pass \d+.*fail 0/i.test(testOut.replace(/\s+/g, ' ')) || (!/fail \d+/.test(testOut) && /pass \d+/.test(testOut));
  } catch (e) {
    testOut = String(e.message || e);
  }
  t('DEV-EXEC 产物真实落盘（sub.js 存在）', subExists, { subExists });
  t('DEV-EXEC 本地复跑 node --test 通过', testPass, { out: testOut.replace(/\s+/g, ' ').slice(0, 160) });
  if (devText) {
    confirmed.push({ stage: 'dev', title: '开发执行结果', text: devText });
    report.external.push({ step: 'dev-exec', len: devText.length, snippet: devText.slice(0, 300) });
  }
  ...
  t('TEST 发送 TEXT 作业（带 req+design+dev EXEC 结果 stageContext）', true, { status: m.status });
  ai = await waitAiOk(pid, 'test', auth);
  t('TEST ai 终态 ok', !!(ai && ai.status === 'ok' && !ai.failed), { meta: ai?.metadata || null });
  if (!ai || ai.failed || !ai.content || ai.content.trim().length < 30) return finish();
  const testText = ai.content.trim();
  t('TEST 产出非空', testText.length > 30, { len: testText.length });
  const testReal = await realFlagOf(ai.id);
  t('TEST metadata.real=true（DB 直查）', testReal === 'true', { realFlag: testReal });
  const testNoClaim = !/未提供上一步|未包含上一步|当前(对话|会话)未(提供|包含|包括)上一步/.test(testText);
  t('TEST 不再声明"未提供上一步内容"', testNoClaim, {});
  // 核心：test 引用 dev EXEC 真实执行结果（sub/减法/node --test/测试通过 等执行证据词）
  const citesExec = /sub\.js|减法|node --test|测试通过|已通过|已执行|执行结果|新增.*测试|测试矩阵.*(依据|基于|根据).*执行/.test(testText);
  t('TEST 产出引用 dev EXEC 执行结果（sub/减法/测试通过）', citesExec, { citesExec });
  ...
  report.external.push({ chainConfirmed: confirmed.map((c) => c.stage + ':' + c.text.length + 'B') });
...
async function failSend(tag, m) {
  let body = '';
  try { body = JSON.stringify(await m.json()); } catch {}
...
function finish() {
  ...
  const out = resolve(REPORT_DIR, 'mixed-chain-' + Math.floor(Date.now() / 1000) + '.json');
...
main().catch((e) => {
  ...
  report.status = 'FAIL';
  writeFileSync(
    resolve(REPORT_DIR, 'mixed-chain-' + Math.floor(Date.now() / 1000) + '.json'),
