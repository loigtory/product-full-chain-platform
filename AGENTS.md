...
- Latest approval: the user replied “同意” to the concrete 27 proposal A and its exact implementation whitelist/test limits. M2c-2 is now LOCAL_VERIFIED (13/13 gates, 36 PG/API/WS and 15 PG browser cases), pending user acceptance; implementation was authorized on `feat/m2c-domain`, tracked in [28](docs/planning/prototype-v3/28-M2c-2实施与验收-20260912.md). This supersedes the docs-only status below. The Express-only future `pfc_workbench` design exception is approved; applying that business schema remains a final separate decision. Execute only the exact disposable schemas, file limits, commands and cleanup in 27/28. Opening/closing feature commits and feature push follow 23; main merge, real tools, old data import and business activation remain excluded.
