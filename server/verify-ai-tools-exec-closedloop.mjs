...
if (process.env.EVIDENCE_ONLY) {
  ...
  try {
    ...
    echeck('evidence job SUCCEEDED', () =>
      assert.equal(ev.job?.state, 'SUCCEEDED'),
    );
    echeck('evidence test fail→pass', () => {
      assert.equal(ev.testBefore?.passed, false);
      assert.equal(ev.testAfter?.passed, true);
    });
    echeck('evidence actual diff', () => assert.equal(ev.diffHappened, true));
    echeck('evidence platform repo clean', () =>
      assert.equal(ev.repoClean, true),
    );
...
const runNodeTest = (cwd) => {
  try {
    const out = execFileSync(nodeBin, ['--test'], {
      cwd,
      encoding: 'utf8',
      timeout: 60000,
...
try {
  ...
  mkdirSync(path.resolve('.local/ai-tools-closedloop-20260915'), {
    recursive: true,
  });
  const base = mkdtempSync(
    path.join(path.resolve('.local/ai-tools-closedloop-20260915'), 'proj_'),
  );
  ...
  const enqueued = await withTransaction(fixture.db, async (client) => {
    ...
    const job = await jobs.enqueue(client, fixture.db, ctx, req, {
      ...
      inputHash: createHash('sha256')
        .update(JSON.stringify({ content: prompt, workspace }))
        .digest('hex'),
      input: {
  ...
  check('actual diff on synthetic project', () =>
    assert.equal(report.diffHappened, true),
  );

  // 平台仓库无新追踪改动（排除闸证据目录；比较跑闸前后差集——A–D 已修改未提交的文件属闸前基线，不计入）
  const gitStatusAfter = execSync('git status --short', {
    cwd: root,
    encoding: 'utf8',
  });
  const norm = (s) =>
  ...
  console.log(
    JSON.stringify({
      ...
      file,
      testBefore: report.testBefore?.passed,
      testAfter: report.testAfter?.passed,
      diffHappened: report.diffHappened,
      repoClean: report.repoClean,
      error: report.error,
    }),
  );
  try {
    rmSync(path.resolve('.local/ai-tools-closedloop-20260915'), {
      recursive: true,
      force: true,
    });
  } catch {
